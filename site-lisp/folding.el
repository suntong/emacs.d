...
;;              Anders Lindgren <andersl@csd.uu.se>
;; Created:     1992
;; RCS version: $Revision: 2.77 $
;; Date:        $Date: 2000/07/25 15:26:14 $
;; Keywords:    tools
...
;;      directory containing this file) and optinally byte compile it.
;;
...
;;      variables before package uses or sees them, and only then load it.
...
;;
;;
;; X.x   Jul  25  2000  20.6             [jari 2.76-2.77]
...
(defvar folding-version  (substring "$Revision: 2.77 $" 11 15)
            ...
            (cond
             ...
             (folding-xemacs-p
		      ...
		      (list (intern
			     (format "button%d"
				     (funcall
				      (symbol-function 'event-button) event)
				     )))))))
             (t
              (error "This version of Emacs can't handle events.")))
...
`folding-internal-margins', which is one by default."
  ...
  (if buffer-read-only nil
	...
	(goto-char (point-min)))
      ...
      (and (if (let (case-fold-search) (looking-at top-re))
	...
	(goto-char (point-max)))
      ...
      (beginning-of-line)
      (and (or (let (case-fold-search) (looking-at folding-bottom-regexp))
